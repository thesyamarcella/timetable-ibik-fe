...
const MasterDataSemesterPage = () => {
  ...
  const handleDeleteData = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(`http://localhost:3000/api/semesters/${id}`);
  ...
  const handleSubmit = async (event) => {
    ...
    if (editMode) {
      try {
        await axios.put(`http://localhost:3000/api/semesters/${formData.id}`, formData);
      ...
      try {
        const response = await axios.post('http://localhost:3000/api/semesters', formData);
        const newData = {
          id: response.data.id,
          name: formData.name,
        };
            ...
            <Col md={6}>
              <Form>
                <Form.Group controlId="searchTerm">
                  <Form.Control type="text" placeholder="Cari Semester" onChange={handleSearch} />
                </Form.Group>
              </Form>
            </Col>
...
export default MasterDataSemesterPage;
